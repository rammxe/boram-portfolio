...
var cw = window.innerWidth,
    ...
    detail = 30,
    waveY  = 0;
var waveStartScrollY = 0, maxWaveY = ch * 1.0;
...
function startTyping() {
  ...
  let index = 0, blinkInterval = null;
  ...
  function triggerAutoExpand() {
    ...
    gsap.fromTo(
      ...
      { scale: isMobile ? mobileCircleScale : currentScale },
      {
        ...
        duration: isMobile ? 1.2 : 2.0,       // ✅ 느리고 부드럽게
        ease: 'power1.inOut',                  // ✅ 천천히 시작 → 천천히 끝
        force3D: true,
        overwrite: true,
        onComplete: () => {
          var waveWrapEl = document.querySelector('.wave-wrap');
    ...
    function onMobileScroll() {
      ...
      if (!autoTriggered) {
        ...
        if (waveSvg) {
          ...
          waveSvg.style.visibility = progress > waveShowProgress ? 'visible' : 'hidden';
        }
        if (waveLetters) {
          waveLetters.style.opacity    = progress > lettersShowProgress ? '1' : '0';
          waveLetters.style.visibility = progress > lettersShowProgress ? 'visible' : 'hidden';
        }
      }
    }
  ...
  /* ===================== 데스크톱 ===================== */
    ...
    heroTl = gsap.timeline({
      scrollTrigger: {
        ...
        onLeave: () => hideWave(),
        onEnterBack: () => {
          autoTriggered = false; heroTl = null; mobileCircleScale = 0;
          var mEl = document.getElementById('m');
          if (mEl) mEl.querySelectorAll('polygon').forEach(p => p.remove());
          waveStarted = false; waves = [];
          gsap.ticker.remove(drawWave);
        ...
        onUpdate: (self) => {
          ...
          // ✅ 0.45에서 자동 확장 트리거 (visibility:hidden 줄 완전 제거)
          if (progress >= 0.45 && !autoTriggered) { triggerAutoExpand(); return; }
          if (!autoTriggered) {
...
function initAboutAnimation() {
  ...
  document.querySelectorAll('.about-panel').forEach(panel => {
    ...
    const tl = gsap.timeline({
      scrollTrigger: { trigger: panel, start: 'top 60%', end: 'bottom 52%', scrub: 0.5 },
    });
    allChars.forEach((char, i) => {
      tl.to(char, { color: '#ffffff', duration: 0.2, ease: 'power2.inOut' }, i * 0.03);
    });
...
/* ✅ 핵심 수정: N개 setTimeout 루프 → rAF 단일 루프로 교체 (렉 제거) */
var randomizeLetters = function (letters) {
  return new Promise(function (resolve) {
    ...
    // 각 글자가 확정될 시간(ms) 미리 계산
    var completionTimes = letters.map(function (_, pos) {
      return pos * lineEq(40, 0, 8, 200, lettersTotal);
    });
    var completed = new Array(lettersTotal).fill(false);

    function tick(timestamp) {
      if (!startTime) startTime = timestamp;
      var elapsed = timestamp - startTime;

      // ✅ rAF 한 프레임에 모든 글자 한꺼번에 처리
...
class Slide {
  ...
  position(pos) {
    gsap.set(this.DOM.imgWrap, {
      x: this.transforms[pos].x, y: this.transforms[pos].y,
  ...
  moveToPosition(settings) {
    const isMobile = winsize.width <= 768;
    const dur = isMobile ? 0.32 : 0.8;   // ✅ 모바일 더 빠르게
    return new Promise(resolve => {
      gsap.to(this.DOM.imgWrap, {
        duration: dur,
        ease:     isMobile ? 'power2.out' : 'power4.inOut',
      ...
      if (settings.resetImageScale) {
        gsap.to(this.DOM.img, { duration: dur, ease: 'power4.inOut', scale: 1 });
      }
    });
  }
  ...
  showTexts(animation) {
    ...
    if (animation) {
      /* ✅ 모바일: randomizeLetters 완전 생략 → 즉시 표시 (렉 핵심 제거) */
      ...
      gsap.to(this.DOM.texts.number, {
        ...
        startAt: { x: '-50%', opacity: 0 }, x: '0%', opacity: 1,
      });
    }
  }
}
...
class Content {
  ...
  show() {
    ...
    gsap.to([this.DOM.number, this.DOM.title, this.DOM.subtitle, this.DOM.info, this.DOM.buttons], {
      duration: 0.8, ease: 'power4.out', opacity: 1,
      startAt: { y: 40, opacity: 0 }, y: 0, stagger: 0.05,
    });
...
class Slideshow {
  ...
  navigate(direction) {
    ...
    /* ✅ 모바일 딜레이 최소화 → 슬라이드 반응 즉각적으로 */
    const d1 = 0,
          d2 = isMobile ? 0.01 : 0.07,
          d3 = isMobile ? 0.02 : 0.14,
          d4 = isMobile ? 0.03 : 0.21;
    ...
    /* ✅ 모바일에서는 텍스트 즉시 표시 (showTexts 내부에서 처리) */
...
function initIllustrationScroll() {
  ...
  items.forEach(function (item) {
    item.addEventListener('mouseenter', function () {
      gsap.to(item, { scale: 1.1, rotationY: 5, rotationX: -3, duration: 0.4, ease: 'power2.out', zIndex: 10, overwrite: 'auto' });
    });
    item.addEventListener('mouseleave', function () {
      gsap.to(item, { scale: 1, rotationY: 0, rotationX: 0, duration: 0.4, ease: 'power2.out', zIndex: 1, overwrite: 'auto' });
    });
  });
}
...
function initNewIntroAnimation() {
  ...
  ScrollTrigger.create({
    ...
    onEnter: function () {
      ...
      if (verticalLine)   setTimeout(() => { verticalLine.classList.add('revealed'); gsap.to(verticalLine, { height: '60%', duration: 0.8, ease: 'power2.out' }); }, 500);
      setTimeout(() => {
...
(function () {
  ...
  var cursorSpeed = 0.15, cursorRafId = null;
  ...
  var animateit = function (e) {
    var span = this.querySelector(':scope > span'); if (!span) return;
    var x = e.offsetX, y = e.offsetY, w = this.offsetWidth, h = this.offsetHeight, move = 25;
    span.style.transform = e.type === 'mouseleave' ? ''
...
(function () {
  ...
  var animateMenuLink = function (e) {
    var span = this.querySelector('span'); if (!span) return;
    var x = e.offsetX, y = e.offsetY, w = this.offsetWidth, h = this.offsetHeight, move = 25;
    span.style.transform = e.type === 'mouseleave' ? ''
      : 'translate(' + ((x/w)*(move*2)-move) + 'px, ' + ((y/h)*(move*2)-move) + 'px)';
  };
