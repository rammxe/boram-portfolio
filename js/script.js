...
function hideWave() {
  ...
  if (waveWrapEl) { waveWrapEl.style.visibility = 'hidden'; waveWrapEl.style.pointerEvents = 'none'; }
...
function startWaveAnimation() {
  ...
  if (mEl) { mEl.querySelectorAll('polygon').forEach(function (p) { p.remove(); }); }
  waves = [];
...
function initScrollAnimation() {
  ...
  function triggerAutoExpand() {
    ...
    gsap.fromTo(expandingCircle,
      { scale: mobileCircleScale },
      {
        ...
        onComplete: () => {
          var waveWrapEl = document.querySelector('.wave-wrap');
          if (waveWrapEl) { waveWrapEl.style.visibility = 'visible'; waveWrapEl.style.pointerEvents = ''; }
          if (!waveStarted) startWaveAnimation();
          if (waveSvg) { waveSvg.style.opacity = '1'; waveSvg.style.visibility = 'visible'; }
          if (waveLetters) { setTimeout(() => { waveLetters.style.opacity = '1'; waveLetters.style.visibility = 'visible'; }, 600); }
        },
      },
    );
  }
  ...
  if (isMobile) {
    ...
    function onMobileScroll() {
      ...
      const progress = Math.min(Math.max(-rect.top / heroIntro.offsetHeight, 0), 1);
      ...
      if (!autoTriggered && blurtext) {
        ...
        blurtext.style.visibility = progress > blurHideProgress ? 'hidden' : 'visible';
      }
      ...
      if (!autoTriggered) {
        ...
        if (waveSvg) { waveSvg.style.opacity = progress > waveShowProgress ? '1' : '0'; waveSvg.style.visibility = progress > waveShowProgress ? 'visible' : 'hidden'; }
        if (waveLetters) { waveLetters.style.opacity = progress > lettersShowProgress ? '1' : '0'; waveLetters.style.visibility = progress > lettersShowProgress ? 'visible' : 'hidden'; }
      }
    }
    ...
    if (introNewSec) {
      ScrollTrigger.create({ trigger: introNewSec, start: 'top 95%', onEnter: () => hideWave() });
    }
  ...
  /* ===================== 데스크톱 ===================== */
    ...
    heroTl = gsap.timeline({
      scrollTrigger: {
        ...
        onLeave: () => hideWave(),
        onEnterBack: () => {
          // triggerAutoExpand 호출 전(60% 미만)에 돌아온 경우
          autoTriggered = false;
          heroTl = null;
        ...
        onUpdate: (self) => {
          ...
          if (expandingCircle && progress > 0.01) expandingCircle.classList.add('show');
          if (scrollHint) scrollHint.classList.toggle('hide', progress > 0.02);
          if (!autoTriggered && blurtext) {
            blurtext.style.opacity = progress > blurHideProgress ? '0' : '1';
            blurtext.style.visibility = progress > blurHideProgress ? 'hidden' : 'visible';
          ...
          if (!autoTriggered) {
            ...
            if (waveSvg) { waveSvg.style.opacity = progress > waveShowProgress ? '1' : '0'; waveSvg.style.visibility = progress > waveShowProgress ? 'visible' : 'hidden'; }
            if (waveLetters) { waveLetters.style.opacity = progress > lettersShowProgress ? '1' : '0'; waveLetters.style.visibility = progress > lettersShowProgress ? 'visible' : 'hidden'; }
            if (expandingCircle) { expandingCircle.style.visibility = progress > 0.55 ? 'hidden' : 'visible'; }
          }
        },
      },
    });
    ...
    // ✅ 수정 1: triggerAutoExpand가 heroTl.scrollTrigger를 kill한 뒤에도
    // 위로 돌아올 때 wave가 복원되도록 별도 독립 ScrollTrigger 추가
    ScrollTrigger.create({
      trigger: heroIntro,
      start: 'top 10%',
      onEnterBack: () => {
        ...
        // blurtext 숨기고 wave 바로 재시작 (이미 한 번 내려간 상태이므로 wave 상태로 복원)
        if (blurtext) { blurtext.style.opacity = '0'; blurtext.style.visibility = 'hidden'; }
        ...
        if (waveSvg) { waveSvg.style.opacity = '1'; waveSvg.style.visibility = 'visible'; }
        if (waveLetters) {
          setTimeout(() => { waveLetters.style.opacity = '1'; waveLetters.style.visibility = 'visible'; }, 600);
        }
      },
    });
    ...
    const introNewSec = document.getElementById('intro-new');
    if (introNewSec) {
      ScrollTrigger.create({ trigger: introNewSec, start: 'top 98%', onEnter: () => hideWave() });
    }
  }
}
...
function initAboutAnimation() {
  function splitToChars(el) {
    ...
    [...text].forEach((char) => {
      ...
      span.style.cssText = 'display:inline-block; color:rgba(255,255,255,0.05); white-space:pre;';
      el.appendChild(span);
    });
      ...
      if (node.nodeType === 3 && node.textContent.trim()) {
        ...
        parts.forEach((w) => {
          if (/^\s+$/.test(w)) { frag.appendChild(document.createTextNode(w)); }
            ...
            const span = document.createElement('span');
            span.className = 'dw'; span.textContent = w;
            span.style.cssText = 'display:inline-block; color:rgba(255,255,255,0.05); font-weight:400;';
        ...
        innerText.split(/(\s+)/).forEach((w) => {
          if (/^\s+$/.test(w)) { node.appendChild(document.createTextNode(w)); }
            ...
            const span = document.createElement('span');
            span.className = 'dw dw--bold'; span.textContent = w;
            span.style.cssText = 'display:inline-block; color:rgba(255,255,255,0.05); font-weight:700;';
  ...
  function killFloatTweens(img) {
    if (img._floatTweens) { img._floatTweens.forEach((t) => t.kill()); img._floatTweens = null; }
  }

  document.querySelectorAll('.about-panel').forEach((panel) => {
    const floatImgs = Array.from(panel.querySelectorAll('.float-img'));
    var allChars = [];
    panel.querySelectorAll('.word').forEach((wordEl) => { allChars.push(...splitToChars(wordEl)); });
    ...
    allChars.forEach((char, i) => { tl.to(char, { color: '#ffffff', duration: 0.2, ease: 'power2.inOut' }, i * 0.03); });
    descWords.forEach((word, i) => {
      const isBold = word.classList.contains('dw--bold');
      tl.to(word, { color: isBold ? '#ffffff' : 'rgba(255,255,255,0.8)', duration: 0.25, ease: 'power1.inOut' }, i * 0.05);
    });
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => {
        floatImgs.forEach((img, i) => {
          ...
          gsap.to(img, { opacity: 1, y: 0, duration: 1.0, delay: i * 0.12, ease: 'power3.out',
            onComplete: () => { img._floatTweens = createFloatTweens(img, floatParams[i] || floatParams[0]); } });
        });
      },
      onLeave: () => {
        floatImgs.forEach((img) => { killFloatTweens(img); gsap.to(img, { opacity: 0, y: -60, duration: 0.5, ease: 'power2.in' }); });
      },
      onEnterBack: () => {
        floatImgs.forEach((img, i) => {
          killFloatTweens(img);
      ...
      onLeaveBack: () => {
        floatImgs.forEach((img) => { killFloatTweens(img); gsap.to(img, { opacity: 0, y: 80, duration: 0.5, ease: 'power2.in' }); });
      },
    });
  });
}
...
var lineEq = function (y2, y1, x2, x1, currentVal) { var m = (y2 - y1) / (x2 - x1); return m * currentVal + (y1 - m * x1); };
...
var charming = function (el) {
  ...
  text.split('').forEach(function (char, i) {
    ...
    span.className = 'char' + (i + 1); span.textContent = char; el.appendChild(span);
  });
};

var randomizeLetters = function (letters) {
  return new Promise(function (resolve) {
    var lettersTotal = letters.length, cnt = 0;
    letters.forEach(function (letter, pos) {
      var loopTimeout;
      var loop = function () { letter.innerHTML = chars[getRandomInt(0, charsTotal - 1)]; loopTimeout = setTimeout(loop, getRandomInt(20, 80)); };
      loop();
      setTimeout(function () {
        clearTimeout(loopTimeout); letter.style.opacity = 1; letter.innerHTML = letter.dataset.initial;
...
var disassembleLetters = function (letters) {
  return new Promise(function (resolve) {
    var lettersTotal = letters.length, cnt = 0;
    letters.forEach(function (letter, pos) {
      setTimeout(function () { letter.style.opacity = 0; if (++cnt === lettersTotal) resolve(); }, pos * 30);
    });
  });
};
...
class Slide {
  ...
  calcTransforms() {
    ...
    if (isMobile) {
      this.transforms = [
        { x: -winsize.width * 1.2, y: 0, rotation: 0 }, { x: -winsize.width * 0.6, y: 0, rotation: 0 },
        { x: 0, y: 0, rotation: 0 }, { x: winsize.width * 0.6, y: 0, rotation: 0 },
        { x: winsize.width * 1.2, y: 0, rotation: 0 }, { x: 0, y: 0, rotation: 0 },
      ];
    } else {
      this.transforms = [
  ...
  initEvents() {
    this.DOM.imgWrap.addEventListener('mouseenter', () => {
      ...
      this.mousetime = setTimeout(() => gsap.to(this.DOM.img, { duration: 0.5, ease: 'power2.out', scale: 1.08 }), 40);
    });
  ...
  setCurrent(isContentOpen) { this.isCurrent = true; this.DOM.el.classList.add('slide--current', 'slide--visible'); this.position(isContentOpen ? 5 : 2); }
  setLeft(isContentOpen) { this.isRight = this.isCurrent = false; this.isLeft = true; this.DOM.el.classList.add('slide--visible'); this.position(isContentOpen ? 0 : 1); }
  setRight(isContentOpen) { this.isLeft = this.isCurrent = false; this.isRight = true; this.DOM.el.classList.add('slide--visible'); this.position(isContentOpen ? 4 : 3); }
    ...
    return new Promise((resolve) => {
      gsap.to(this.DOM.imgWrap, {
        ...
        startAt: settings.from !== undefined
          ? { x: this.transforms[settings.from + 2].x, y: this.transforms[settings.from + 2].y, rotationX: 0, rotationY: 0, rotationZ: this.transforms[settings.from + 2].rotation }
          : {},
      ...
      if (settings.resetImageScale) { gsap.to(this.DOM.img, { duration: dur, ease: 'power4.inOut', scale: 1 }); }
    });
  }
  hideTexts(animation) {
    ...
    if (animation) {
      ...
      gsap.set(this.DOM.texts.wrap, { opacity: 0 }); gsap.set(this.DOM.texts.side, { opacity: 0 });
    }
  }
  showTexts(animation) {
    animation = animation !== false;
    gsap.set(this.DOM.texts.wrap, { opacity: 1 }); gsap.set(this.DOM.texts.side, { opacity: 1 });
    if (animation) {
      randomizeLetters(this.DOM.titleLetters); randomizeLetters(this.DOM.sideLetters);
...
class Content {
  ...
  show() {
    // ✅ 수정 2: getAttribute로 정확히 체크 후 클릭 시 개별 로드
    ...
    document.querySelectorAll('.content .marquee-track--left, .content .marquee-track--right').forEach(function (track) {
      track.innerHTML = Array(20).fill('<span>' + projectName + '</span>').join('');
    });
...
class Slideshow {
  ...
  bounceDeco(direction, delay) {
    ...
    gsap.to(this.DOM.deco, {
      ...
      x: direction === 'next' ? -40 : 40, y: direction === 'next' ? -40 : 40,
  ...
  navigate(direction) {
    ...
    const isMobile = winsize.width <= 768;
    const d1 = 0, d2 = isMobile ? 0.05 : 0.07, d3 = isMobile ? 0.1 : 0.14, d4 = isMobile ? 0.14 : 0.21;
    ...
    this.prevSlide.moveToPosition({ position: direction === 'next' ? -2 : 0, delay: direction === 'next' ? d1 : d3 })
      .then(() => { if (direction === 'next') this.prevSlide.hide(); });
    this.currentSlide.moveToPosition({ position: direction === 'next' ? -1 : 1, delay: d2 });
    this.currentSlide.hideTexts();
    this.bounceDeco(direction, d2);
    this.nextSlide.moveToPosition({ position: direction === 'next' ? 0 : 2, delay: direction === 'next' ? d3 : d1 })
      .then(() => { if (direction === 'prev') this.nextSlide.hide(); });
    if (direction === 'next') this.nextSlide.showTexts();
    else this.prevSlide.showTexts();
    this.upcomingSlide.moveToPosition({ position: direction === 'next' ? 1 : -1, from: direction === 'next' ? 2 : -2, delay: d4 })
      .then(() => {
...
// ✅ 수정 3: 비디오 일괄 로드 제거 (Content.show()에서 개별 로드)
...
function initIllustrationScroll() {
  ...
  gsap.set(titleLeft, { x: '-50vw' }); gsap.set(titleRight, { x: '50vw' });
  var titleConfig = { trigger: gallerySection, start: 'top 95%', end: 'top 10%', scrub: 3 };
  ...
  ScrollTrigger.create({
    trigger: gallerySection, start: 'top 80%', end: 'bottom top',
  ...
  gsap.timeline({ scrollTrigger: { trigger: gallerySection, start: 'top top', end: '+=300', scrub: 0.3, pin: true } })
    .to(items[0], { x: '-34vw', y: 0, scale: 1 }, 0)
    .to(items[1], { x: '0vw', y: 0, scale: 1 }, 0)
    .to(items[2], { x: '34vw', y: 0, scale: 1 }, 0);
  items.forEach(function (item) {
    item.addEventListener('mouseenter', function () { gsap.to(item, { scale: 1.1, rotationY: 5, rotationX: -3, duration: 0.4, ease: 'power2.out', zIndex: 10, overwrite: 'auto' }); });
    item.addEventListener('mouseleave', function () { gsap.to(item, { scale: 1, rotationY: 0, rotationX: 0, duration: 0.4, ease: 'power2.out', zIndex: 1, overwrite: 'auto' }); });
  });
}
...
function initContactAnimation() {
  ...
  gsap.to(gradientImg, { scale: 3, ease: 'none', scrollTrigger: { trigger: contactSection, start: 'top bottom', end: 'center center', scrub: 1.2 } });
  if (img) { gsap.to(img, { scale: 1.25, ease: 'none', scrollTrigger: { trigger: contactSection, start: 'top bottom', end: 'bottom top', scrub: 2 } }); }
  ScrollTrigger.create({
    trigger: contactSection, start: 'top 60%',
    onEnter: () => { if (title) title.classList.add('revealed'); if (desc) desc.classList.add('revealed'); if (btn) btn.classList.add('revealed'); },
    onLeaveBack: () => { if (title) title.classList.remove('revealed'); if (desc) desc.classList.remove('revealed'); if (btn) btn.classList.remove('revealed'); },
  });
}
...
function initNewIntroAnimation() {
  ...
  ScrollTrigger.create({
    trigger: introNewSection, start: 'top 75%', once: true,
    onEnter: function () {
      if (introNewImage) setTimeout(() => introNewImage.classList.add('revealed'), 100);
      if (orbitRight) setTimeout(() => orbitRight.classList.add('revealed'), 300);
      if (verticalLine) {
        setTimeout(() => { verticalLine.classList.add('revealed'); gsap.to(verticalLine, { height: '60%', duration: 0.8, ease: 'power2.out' }); }, 500);
      }
      setTimeout(() => {
...
function initProjectsTitleAnimation() {
  ...
  gsap.set(titleLeft, { x: '-50vw' }); gsap.set(titleRight, { x: '50vw' });
  var stConfig = { trigger: projectsSection, start: 'top 95%', end: 'top 10%', scrub: 3 };
...
window.addEventListener('resize', function () {
  cw = window.innerWidth; ch = window.innerHeight; calcWinsize();
...
(function () {
  ...
  var animateit = function (e) {
    ...
    if (!span) return;
    var x = e.offsetX, y = e.offsetY, w = this.offsetWidth, h = this.offsetHeight, move = 25;
    span.style.transform = e.type === 'mouseleave' ? '' : 'translate(' + ((x / w) * move * 2 - move) + 'px,' + ((y / h) * move * 2 - move) + 'px)';
  };
  var interactiveEls = Array.from(document.querySelectorAll('a, button, .slide__img-wrap, .nav')).filter(
    (el) => !el.closest('.dropdown-menu') && !el.closest('.menu-btn') && !el.closest('.menu-wrapper'),
  );
...
(function () {
  ...
  menuLinks.forEach(function (link) {
    ...
    link.addEventListener('click', function () {
      menuToggle.checked = false; document.body.classList.remove('menu-open');
...
function triggerOutAnimation() {
  ...
  document.querySelectorAll('.out-box').forEach(function (box, i) { setTimeout(() => box.classList.add('revealed'), i * 200); });
  document.querySelectorAll('.out-item').forEach(function (item, i) { setTimeout(() => item.classList.add('revealed'), 400 + i * 120); });
  document.querySelectorAll('.badge').forEach(function (badge, i) { setTimeout(() => badge.classList.add('revealed'), 700 + i * 100); });
}
...
function triggerSkillAnimation() {
  ...
  if (skillTitle) { skillTitle.style.transition = 'opacity 0.6s ease,transform 0.6s ease'; skillTitle.style.opacity = '1'; skillTitle.style.transform = 'translateY(0)'; }
  document.querySelectorAll('.skill-item').forEach(function (item, i) { setTimeout(() => item.classList.add('revealed'), 200 + i * 80); });
}
...
prevBtn.addEventListener('click', function () { if (currentPage > 0) { currentPage--; updateNavigation(); } });
nextBtn.addEventListener('click', function () { if (currentPage < totalPages - 1) { currentPage++; updateNavigation(); } });

(function () {
  if (!wrapper) return;
  var startX = 0, startY = 0, isDragging = false;
  wrapper.addEventListener('touchstart', function (e) { startX = e.touches[0].clientX; startY = e.touches[0].clientY; isDragging = true; }, { passive: true });
