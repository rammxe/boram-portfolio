...
function initScrollAnimation() {
  ...
  let heroTl           = null;
  let heroST           = null; // ScrollTrigger 별도 보관
  let mobileCircleScale= 0;
  const introNew       = document.getElementById('intro-new');

  /* ─ 원 자동확장 ─
     kill 대신 disable → 스크롤 점프 없음
     onComplete 후 intro-new로 부드럽게 이동 → 검은화면 없음
  ─────────────────── */
  function triggerAutoExpand() {
    if (autoTriggered) return;
    autoTriggered = true;

    // ✅ kill 아닌 disable(false) → spacer 유지, 스크롤 점프 방지
    if (heroST) heroST.disable(false);
    ...
    gsap.fromTo(expandingCircle,
      { scale: isMobile ? mobileCircleScale : currentScale },
      {
        ...
        onComplete: () => {
          ...
          // ✅ 웨이브 보여주고 → intro-new로 부드럽게 이동 (검은화면 제거)
          setTimeout(() => {
          ...
          }, isMobile ? 2000 : 2500); // ✅ PARK BO RAM 타이핑 잠깐 보여주고 이동
        },
      },
    );
  }
  ...
  /* ──────── 데스크톱 ──────── */
    ...
    heroTl = gsap.timeline({
      scrollTrigger: {
        ...
        onEnterBack: () => {
          autoTriggered = false;
          if (heroST) heroST.enable();    // ✅ 다시 활성화
          resetWaveState();
          if (expandingCircle) { gsap.set(expandingCircle, { scale: 0, force3D: true }); expandingCircle.classList.remove('show'); }
          if (blurtext) { blurtext.style.opacity = '1'; blurtext.style.visibility = 'visible'; }
          blurItems.forEach(item => item.classList.remove('active', 'past'));
    ...
    // ✅ ScrollTrigger 참조 저장 (disable/enable 용)
