...
/* ✅ hideWave: 완전히 즉각 숨김 (transition 없이) */
function hideWave() {
  if (waveSvg) {
...
function initScrollAnimation() {
  ...
  /* ✅ 모바일 원형 scale 축소: 80 → 15 (GPU 부담 대폭 감소) */
  const circleScale = isMobile ? 15 : 80;
  ...
  const tl = gsap.timeline({
    scrollTrigger: {
      ...
      onLeave: () => hideWave(),
      onEnterBack: () => {
        /* 히어로로 다시 돌아왔을 때 hero-content 다시 보이게 */
        if (heroContent) {
  ...
  /* ✅ intro-new 섹션 진입 시 wave 강제 즉시 숨김 */
