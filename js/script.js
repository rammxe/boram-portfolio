...
function initScrollAnimation() {
  ...
  const waveShowProgress = isMobile ? 0.3 : 0.4;
  const lettersShowProgress = isMobile ? 0.42 : 0.65;
  const blurHideProgress = isMobile ? 0.18 : 0.3;

  /* =====================================================
     ✅ 모바일: pin 없이 scrub:true, 원형은 독립 트리거
     ===================================================== */
  if (isMobile) {
    /* 원형 - scrub과 완전 분리, 독립 GSAP 애니메이션 */
    ...
    /* 나머지 hero 효과 - scrub:true로 저항감 제거 */
    ScrollTrigger.create({
      trigger: heroIntro,
      start: 'top top',
      end: 'bottom bottom',
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        onUpdate: (self) => {
          ...
          if (expandingCircle) {
            expandingCircle.style.visibility = progress > 0.55 ? 'hidden' : 'visible';
          }
        },
      },
    });
    tl.to(expandingCircle, { scale: 80, duration: 1.5, ease: 'power2.inOut', force3D: true }, 0);
  }

  /* intro-new 진입 시 wave 강제 숨김 */
    ...
    ScrollTrigger.create({
      ...
      onLeave: () => {
        floatImgs.forEach((img) => {
          killFloatTweens(img);
          gsap.to(img, { opacity: 0, y: -60, duration: 0.5, ease: 'power2.in' });
        });
      },
      onEnterBack: () => {
        floatImgs.forEach((img, i) => {
...
class Slide {
  ...
  position(pos) {
    gsap.set(this.DOM.imgWrap, {
      ...
      rotationX: 0, rotationY: 0, opacity: 1, rotationZ: this.transforms[pos].rotation,
    });
  }
    ...
    return new Promise((resolve) => {
      gsap.to(this.DOM.imgWrap, {
        ...
        startAt: settings.from !== undefined ? {
          x: this.transforms[settings.from + 2].x, y: this.transforms[settings.from + 2].y,
          rotationX: 0, rotationY: 0, rotationZ: this.transforms[settings.from + 2].rotation,
        } : {},
...
function createSlideshowWithVideoEvents() {
  document.querySelectorAll('.content__item-image video').forEach(function (video) {
    ...
    if (source && source.dataset.src && !source.src) { source.src = source.dataset.src; video.load(); }
  });
...
function initIllustrationScroll() {
  ...
  gsap.timeline({
    scrollTrigger: { trigger: gallerySection, start: 'top top', end: '+=300', scrub: 0.3, pin: true },
  })
...
function initContactAnimation() {
  ...
  gsap.to(gradientImg, {
    scale: 3, ease: 'none',
    scrollTrigger: { trigger: contactSection, start: 'top bottom', end: 'center center', scrub: 1.2 },
  });
  if (img) {
    gsap.to(img, {
      scale: 1.25, ease: 'none',
      scrollTrigger: { trigger: contactSection, start: 'top bottom', end: 'bottom top', scrub: 2 },
    });
  }
  ScrollTrigger.create({
...
(function () {
  ...
  var interactiveEls = Array.from(document.querySelectorAll('a, button, .slide__img-wrap, .nav'))
...
/* ✅ 화살표: 첫/마지막 페이지만 한쪽 숨김, 중간은 양쪽 다 보임 */
...
prevBtn.addEventListener('click', function () {
  if (currentPage > 0) { currentPage--; updateNavigation(); }
});
nextBtn.addEventListener('click', function () {
  if (currentPage < totalPages - 1) { currentPage++; updateNavigation(); }
});
