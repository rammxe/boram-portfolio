...
function startTyping() {
  ...
  if (typingBlinkInterval) {
    clearInterval(typingBlinkInterval);
    typingBlinkInterval = null;
  }
  ...
  function typeLetter() {
    ...
    if (index < typingText.length) {
      ...
      let visible = true;
      typingBlinkInterval = setInterval(() => {
        if (typingCancelled) {
          clearInterval(typingBlinkInterval);
          return;
        }
...
function initScrollAnimation() {
  ...
  if (isMobile) {
    ...
    heroTl = gsap.timeline({
      scrollTrigger: {
        ...
        onUpdate: (self) => {
          ...
          if (expandingCircle && progress > 0.01)
            expandingCircle.classList.add('show');
          if (scrollHint) scrollHint.classList.toggle('hide', progress > 0.02);
          if (!autoTriggered && blurtext) {
            blurtext.style.opacity = progress > blurHideProgress ? '0' : '1';
    ...
    if (expandingCircle) {
      heroTl.to(
        expandingCircle,
        { scale: 50, duration: 1.5, ease: 'power1.inOut', force3D: true },
        0,
      );
    }

    // ✅ triggerAutoExpand가 heroTl.scrollTrigger를 kill한 뒤에도
  ...
  return new Promise(function (resolve) {
    ...
    letters.forEach(function (letter, pos) {
      ...
      var loop = function () {
        ...
        loopTimeout = setTimeout(loop, getRandomInt(20, 80));
      };
      loop();
      setTimeout(
        function () {
...
class Content {
  ...
  show() {
    // ✅ getAttribute로 정확히 체크 후 클릭 시 개별 로드
